...
class TooltipPortal extends React.Component {
  ...
  findPortalTarget () {
    ...
    const closestHeader = window.isDesktop ? target.closest('.App__desktop__header') : target.closest('.PanelHeader__in');
    const closestPanel = window.isDesktop ?  document.querySelector('.Desktop__content') : this.document.getElementById(this.context.panel).childNodes[0];
  ...
  render () {
    ...
    const offset = window.isDesktop ? 10 : 20;
    return ReactDOM.createPortal(
      <div className={utils.classNames(baseClassName, {
        ...
        <div className="Tooltip__container" style={{ top: this.state.y, left: this.state.x }} ref={this.getRef}>
          <div className="Tooltip__corner" style={{ [alignX]: offset + cornerOffset }}/>
